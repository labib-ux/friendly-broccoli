...
Centralized configuration provider loading values from .env securely.
Responsible for exporting global constants, API keys, and modeling limits 
to external scopes. Validates startup parameters safely avoiding immediate 
fatal execution crashes inside loosely coupled architectures.
"""
...
# Must actively grab the env cache payload BEFORE evaluating helper gets
load_dotenv()


def _get_env(key: str, default: str = "", required: bool = False) -> str:
    """
    Retrieves an environment variable with optional requirement enforcement.

    Args:
        key: The environment variable name.
        default: Value to return if the key is not set.
        required: If True and the key is missing or empty,
                  logs a WARNING — does not raise, so the app
                  can still start in backtest/paper mode without
                  all keys present.
...
INITIAL_CAPITAL: float = float(
    _get_env("INITIAL_CAPITAL", default="10000.0")
)

MAX_POSITION_SIZE: float = float(
    _get_env("MAX_POSITION_SIZE", default="0.2")
)

MAX_DAILY_DRAWDOWN: float = float(
    _get_env("MAX_DAILY_DRAWDOWN", default="0.05")
)
...
def _validate_config() -> None:
    """
    Runs lightweight validation on loaded config values.
    
    Logs warnings for suspicious values rather than raising
    exceptions — the system should degrade gracefully rather
    than refuse to start entirely.
    """
