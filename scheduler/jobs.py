...
def run_trading_cycle(
    ...
    """
    Executes a single live market trading cycle.

    This function wraps the end-to-end inference request natively handling
    data ingestion, dynamic indicator alignment, scaled predictions,
    risk limitation via the circuit breaker, and actual order execution.
    All errors are gracefully caught locally so that standard scheduling
    systems do not permanently kill the scheduled cron job.

    Args:
        adapter: Brokerage interface (e.g. AlpacaAdapter, PolymarketAdapter).
        agent: The RL TradingAgent to generate 'hold/buy/sell' decisions.
        risk_manager: Engine handling position sizing and drawdowns.
        session_maker: SQLAlchemy session factory context wrapper.
        symbol: Ticker format to ingest and execute trades against.
        timeframe: API specific string determining bar resolution.
        lookback_window: Amount of past historical sequence the agent expects.
        initial_capital: Baseline portfolio size used to normalize capital ratio.
    ...
    try:
        ...
        if df is None or df.empty:
            logger.error(
                ...
                "for %s %s — aborting cycle.", symbol, timeframe
            )
            return

        # STEP 2 — Compute technical indicator features
        df = compute_features(df)
        if df is None or df.empty:
            logger.error(
                "compute_features returned empty DataFrame "
                "for %s — aborting cycle.", symbol
            )
            return

        # STEP 3 — Slice to the required observation window
        df = df.tail(lookback_window).copy()
        if len(df) < lookback_window:
            logger.error(
                "Insufficient rows after slicing: got %d, need %d "
                "for %s — aborting cycle.", len(df), lookback_window, symbol
            )
            return

        # STEP 4 — Load scaler and scale features
        scaler_path = os.path.join(
            os.path.dirname(MODEL_SAVE_PATH), "feature_scaler.pkl"
        )
        ...
        try:
            ...
            logger.critical("Unexpected error loading feature scaler: %s", e, exc_info=True)
            return
            
        df, _ = scale_features(df, scaler=scaler)
        if df.empty:
            logger.error("Dataframe empty after scaling — aborting cycle for %s", symbol)
            return

        # STEP 5 — Fetch current portfolio state and price
        ...
        # STEP 6 — Circuit breaker check (must run BEFORE any trade logic)
        if risk_manager.check_circuit_breaker(
            ...
            logger.critical(
                ...
                port['total_value'], day_start_value
            )
            return

        # STEP 7 — Construct the RL observation vector
        ...
        logger.info(
            ...
            bool(position_held), port['cash']
        )

        # STEP 9 — Execute order logic and log to database
        with session_maker() as session:
            log_decision(session, {
                "state_hash": "live",
                "action": action,
                "confidence": 1.0,
                "reward": None
            ...
            if action == 1 and position_held == 0.0:
                ...
                if size > 0:
                    ...
                    logger.info("BUY order submitted: %.6f %s @ $%.4f", size, symbol, current_price)
                else:
                    logger.warning(
                        "BUY signal but calculated position size is 0 "
                        "— order skipped. Capital: $%.2f Price: $%.4f",
                        port['cash'], current_price
                    )
                ...
                logger.info(
                    ...
                    "Realized PnL: $%.4f",
                    position_qty, symbol, current_price, realized_pnl
                )

            elif action == 1 and position_held > 0.0:
                logger.debug("BUY signal ignored — already holding position in %s.", symbol)

            elif action == 2 and position_held == 0.0:
                logger.debug("SELL signal ignored — no position held in %s.", symbol)

    except Exception as e:
        logger.error(
            "run_trading_cycle failed unconditionally. Dropping cycle gracefully. "
